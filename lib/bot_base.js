    ...
    if (config.perGreetings) {
        for (var user in ttRoom.userList) {
            ...
            if (isRoomMod(userObj.userid)) {
                if (auto) {//exclude mods that dont want automated pms (joined/lamer, etc)
                    if ((user.userid != 'xxxx'/*xxxx*/) && (user.userid != 'xxxx'/*xxxx*/)) {
    ...
    switch (commandObj.command) {
        ...
        case 'commands':
            ...
            if (spkr.isMod || spkr.isOwner || spkr.isJBIRD) {
                botSpeak('pm', "***MOD clearq, remq X, uptime, userlist, status, setnewtrack(true/false), crowdcheck(true/false), setcrowdcheck, "
