    ...
    if (config.perGreetings) {
        for (var user in ttRoom.userList) {
            ...
            if (isRoomMod(userObj.userid)) {
                if (auto) {
                    if ((user.userid != '4ea9a8dfa3f751271d03604b'/*FAP*/) && (user.userid != '4e04c8f3a3f751760501d5f4'/*JLBRUNO*/)) {
    ...
    if (config.usedb && config.useDjq) {
        client.query('SELECT userid, username FROM ' + config.botName + 'djq', function (err, results) {
            ...
            if (reUserid === userid) {
                ...
                botSpeak('chat', "@" + username + " you're not first on the q." + reUsername);
                //                bot.remDj(userid);
                getFirstonQ();
            }
        });
...
var ModRemDjq = function (username) {
    if (config.usedb) {
        client.query('DELETE FROM ' + config.botName + 'djq WHERE username = ?', [username], function (err, results) {
...
var getAllowedDjs = function () {
    if (config.usedb) {
        ...
        client.query('SELECT username FROM ' + config.botName + 'alloweddjs ORDER BY id ASC', function (err, results) {
            ...
            botSpeak('chat', response, config.botOwner);
        });
    };
};
...
//dbfunctions
var greetNewUser = function (user) {
    if (config.usedb) {
        ...
        client.query(cmd, function (err, results) {
            if (err) {
                botSpeak('console', err + " greetNewUser");
            }
            if (!username.match(/ttstats/i)) {
                if ((config.perGreetings) && (results) && (results.length === 0)) {
            ...
            + " hr DJ limit. Go ahead and get yourself on the DJ queue with a jq+ in chat", user.name);
    ...
    switch (commandObj.command) {
        ...
        case 'commands':
            ...
            if (spkr.isMod || spkr.isOwner || spkr.isJBIRD) {
                botSpeak('pm', "***MOD clearq, remq X, uptime, userlist, setnewtrack(true/false), crowdcheck(true/false), setcrowdcheck, "
                ...
                + "autobop(true/false), autobopstat, setlimit X, setdecklimit X, songlimitstat, decklimitstat,current, lame, dj, theme X, "
        ...
        case 'rules':
            if (config.deckcheck) {
                botSpeak('chat', config.roomTheme + " please, " + config.songLimit + " minute song limit at this time, 15min AFK rule, "
        ...
        case 'smoke':
            if (config.danceMode) {
                bot.vote('up');
                botSpeak('chat', ":herb:SMOKE 'M IF YOU GOT 'EM!:herb:", commandObj.pmID);
            }
            break;
        case 'emoji':
        ...
        case 'newgreets':
            if (spkr.isOwner || spkr.isJBIRD) {
                ...
                botSpeak(where, 'newgreets set to: ' + config.newgreets, commandObj.pmID);
        ...
        case 'pgreetsstat':
            if (spkr.isOwner || spkr.isJBIRD) {
                botSpeak(where, 'greetings set to: ' + config.perGreetings, commandObj.pmID);
            } else {
                botSpeak(where, "I'm sorry that is a Mod command.", commandObj.pmID);
            }
            break;
        ...
        case 'songlimitstat':
            if (spkr.isMod || spkr.isOwner || spkr.isJBIRD) {
                botSpeak(where, 'Song limit is ' + config.songLimit + " and set to " + config.songTimer, commandObj.pmID);
            } else {
                botSpeak(where, "I'm sorry that is a Mod command.", commandObj.pmID);
            }
            break;
        ...
        case 'decklimitstat':
            if (spkr.isMod || spkr.isOwner || spkr.isJBIRD) {
                botSpeak(where, 'Deck limit is ' + config.deckHours, commandObj.pmID);
            } else {
                botSpeak(where, "I'm sorry that is a Mod command.", commandObj.pmID);
            }
            break;
        ...
        case 'setentersay':
            ...
            if (spkr.isOwner || spkr.isJBIRD) {
                config.enterPM = true;
                config.enterMsg = Msg;
                botSpeak(where, "enter pm set to: " + config.enterPM + ": " + Msg, commandObj.pmID);
                setEnterMsg(Msg);
            }
            break;
        case 'enterstat':
